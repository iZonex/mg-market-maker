...
fn ofi_z_saturates_near_one_on_one_sided_stream() {
    ...
    assert!(
        z <= dec!(1.5),
        "z should be bounded near signal/RMS, got {z}"
    );
}
...
fn ofi_z_near_zero_on_balanced_tape() {
    ...
    assert!(
        z.abs() < dec!(0.5),
        "balanced tape z should be near 0, got {z}"
    );
}
...
fn learned_mp_round_trip() {
    ...
    let src = MomentumSignals::new(10).with_learned_microprice(src_model);
    let mut dst = MomentumSignals::new(10).with_learned_microprice(dst_model);
    let snap = src.snapshot_state().expect("has data");
    dst.restore_state(&snap).unwrap();
