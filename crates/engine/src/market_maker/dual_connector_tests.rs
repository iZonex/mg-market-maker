...
async fn spawn_leverage_setup_calls_connector_on_leverage_node() {
    ...
    // full validator path).
    let conn_trait: Arc<dyn mm_exchange_core::connector::ExchangeConnector> = mock;
...
fn atomic_bundle_ack_sweep_honours_cross_venue_dashboard_signal() {
    ...
    assert!(
        !m && !h,
        "completed bundle's dashboard entry must be cleared"
    );
}
...
fn avellaneda_per_node_gamma_override_matches_direct_config() {
    ...
    let overrides = MarketMakerEngine::build_strategy_node_configs(&g, &baseline);
...
fn avellaneda_as_prob_override_matches_direct_ctx() {
    ...
    let ctx_overrides = MarketMakerEngine::build_strategy_ctx_overrides(&g);
    let ov = ctx_overrides
        .get(&node_id)
        .expect("as_prob override captured");
    let mut ctx_b = parity_ctx(&book, &product, &cfg);
...
fn atomic_bundle_checkpoint_round_trips() {
    ...
    let primary_b = Arc::new(MockConnector::new(VenueId::Binance, VenueProduct::Spot));
...
fn sprint4_strategy_graph_deploy_metrics() {
    ...
    g.edges.push(mm_strategy_graph::Edge {
        from: mm_strategy_graph::PortRef {
            node: strat,
            port: "quotes".into(),
        },
        to: mm_strategy_graph::PortRef {
            node: qout,
            port: "quotes".into(),
        },
    });
    ...
    engine
        .swap_strategy_graph(&g)
        .expect("clean graph compiles");
