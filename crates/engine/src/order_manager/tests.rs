...
fn mock_connector(max_batch: usize) -> Arc<dyn ExchangeConnector> {
    Arc::new(MockConnector::new(VenueId::Bybit, VenueProduct::Spot).with_max_batch_size(max_batch))
...
fn paper_match_trade_filtered_honours_queue_gate() {
    ...
    let fills = mgr.paper_match_trade_filtered(dec!(76_150), Side::Buy, |id| {
        if id == front_id {
            Some(front_price)
        } else {
            None
...
fn paper_match_trade_filtered_fires_all_when_gate_open() {
    ...
    mgr.track_order(a);
    mgr.track_order(b);
    let prices = std::collections::HashMap::from([(a_id, dec!(76_100)), (b_id, dec!(76_050))]);
    let fills =
        mgr.paper_match_trade_filtered(dec!(76_150), Side::Buy, |id| prices.get(&id).copied());
    assert_eq!(fills.len(), 2);
}
...
fn paper_match_trade_filtered_respects_price_override() {
    ...
    let fills = mgr.paper_match_trade_filtered(dec!(76_150), Side::Buy, |_| Some(slipped));
    assert_eq!(fills.len(), 1);
