...
fn rebalance_execute_state_roundtrip_intra_venue() {
    ...
    let dir = std::env::temp_dir().join(format!("mm-rebal-exec-test-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&dir).unwrap();
    let log_path = dir.join("transfers.jsonl");
    let log = mm_persistence::transfer_log::TransferLogWriter::open(&log_path).unwrap();
    ...
    let rows = mm_persistence::transfer_log::read_all(&log_path).unwrap();
